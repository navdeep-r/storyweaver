          ...
          <div className="absolute inset-y-0 right-0 max-w-full flex">
            <div className="relative w-screen max-w-xs sm:max-w-md">
              <div className="h-full flex flex-col bg-white shadow-xl">
                <div className="flex-1 overflow-y-auto py-4 sm:py-6 px-3 sm:px-4 md:px-6">
                  <div className="flex items-start justify-between">
                    <h2 className="text-lg font-medium text-slate-900">Your Selection</h2>
                    <button
                      onClick={() => setIsOpen(false)}
                      className="ml-2 sm:ml-3 h-6 sm:h-7 flex items-center justify-center rounded-md text-slate-400 hover:text-slate-500"
                    >
                  ...
                  <div className="mt-4 sm:mt-8">
                    {cart.length === 0 ? (
                      <div className="text-center py-8 sm:py-12">
                        <ShoppingBagIcon className="mx-auto h-10 w-10 sm:h-12 sm:w-12 text-slate-400" />
                        <h3 className="mt-2 text-sm font-medium text-slate-900">Your cart is empty</h3>
                        <p className="mt-1 text-xs sm:text-sm text-slate-500">Add some books to get started.</p>
                      </div>
                    ) : (
                      <div className="flow-root">
                        <ul className="-my-4 sm:-my-6 divide-y divide-slate-200">
                          {cart.map((item, index) => (
                            <li key={index} className="py-4 sm:py-6 flex">
                              <div className="shrink-0 w-16 h-20 sm:w-24 sm:h-32 bg-slate-200 rounded-md overflow-hidden">
                                {item.book.coverUrl ? (
                                  <img
                ...
                {cart.length > 0 && (
                  <div className="border-t border-slate-200 py-4 sm:py-6 px-3 sm:px-4 md:px-6">
                    ...
                    <div className="mt-4 sm:mt-6">
                      <button
                        ...
                        className="w-full flex justify-center items-center px-4 py-2 sm:px-6 sm:py-3 border border-transparent rounded-md shadow-sm text-sm sm:text-base font-medium text-white bg-blue-600 hover:bg-blue-700"
                      >
                        Download Selected
                      </button>
                    </div>
